...
#pragma omp parallel for schedule(static) 
    ...
    for (int it = 0; it < totalIterations; it++)
    {
        ...
        {
            // 같은 색의 바디들 서로 force를 공유하지 않음(그래프 색칠 보장)
#pragma omp parallel for schedule(static)
            for (int bi = 0; bi < (int)bucket.size(); ++bi)
            {
                ...
                {
                    // 같은 color 안에서 force가 두 번 호출되지 않도록 coloring이 보장
                    ...
                    for (int i = 0; i < force->rows(); ++i)
                    {
                        ...
                        float fmag = clamp(force->penalty[i] * force->C[i] + lambda, force->fmin[i], force->fmax[i]);

                        // lumped geometric stiffness (Sec 3.5)
                        float3x3 G = diagonal(length(force->H[i].col(0)),
                            length(force->H[i].col(1)),
                            length(force->H[i].col(2))) * fabsf(fmag);
                ...
                // Eq. 4
                float3 dx = solve(lhs, rhs);
                body->position -= dx; // in-place (같은 색 안에서는 충돌 없음)
            }
        }
