...
struct Force
{
    ...
    std::atomic<bool> kill{ false };   // 병렬 루프에서 set
    void markForRemoval() { kill.store(true, std::memory_order_relaxed); }
    bool toRemove() const { return kill.load(std::memory_order_relaxed); }
};
